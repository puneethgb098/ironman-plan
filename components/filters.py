...
from config import SPORTS, SPORT_LABELS, WORKOUT_STATUSES, STATUS_LABELS
...
def render_sport_filters(key_prefix: str = "filter") -> list[str]:
    ...
    for sport in ["swim", "bike", "run", "strength", "mobility"]:
        ...
        if st.sidebar.checkbox(f"{label} ({sport})", value=True, key=f"{key_prefix}_sport_{sport}"):
...
def render_status_filters(key_prefix: str = "filter") -> list[str]:
    ...
    for status in WORKOUT_STATUSES:
        ...
        if st.sidebar.checkbox(f"{label}", value=True, key=f"{key_prefix}_status_{status}"):
