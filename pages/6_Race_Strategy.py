...
st.set_page_config(page_title="Race Strategy | IronPlan", page_icon="🏁", layout="wide")
...
st.markdown(f"""
<div class="countdown-box" style="margin-bottom:24px;">
    <div class="countdown-number">{dtrace}</div>
    <div class="countdown-label">Days to {RACE_NAME}</div>
    <div style="font-size:0.9rem; opacity:0.8; margin-top:4px;">{RACE_DATE.strftime('%A, %B %d, %Y')}</div>
</div>
""", unsafe_allow_html=True)

tab1, tab2, tab3, tab4 = st.tabs(["🎯 Race Plan", "⏱️ Pacing", "🔄 Transitions", "⚠️ Contingencies"])

with tab1:
    st.markdown("### Target Race Time: 6:30 – 7:30")

    segments = [
        {"icon": "🏊", "name": "Swim 1.9 km", "target": "45–55 min", "pace": "~2:30–2:55/100m",
         "strategy": "Survival pace. Start at back/side. Backstroke as backup. Sight every 6–8 strokes."},
        {"icon": "🔄", "name": "T1", "target": "5 min", "pace": "—",
         "strategy": "Calm, don't rush. Remove wetsuit if worn. Helmet on first. Sip water."},
        {"icon": "🚴", "name": "Bike 90 km", "target": "3:15–3:30", "pace": "23–25 km/h",
         "strategy": "Steady RPE 5–6. Don't chase. Eat every 20 min, drink every 15 min."},
        {"icon": "🔄", "name": "T2", "target": "3 min", "pace": "—",
         "strategy": "Quick change. Gel + water before heading out."},
        {"icon": "🏃", "name": "Run 21.1 km", "target": "2:10–2:30", "pace": "6:10–7:00/km",
         "strategy": "Walk out of T2. Settle km 3–10. Walk aid stations. Run-walk if needed."},
    ]

    for seg in segments:
        border_color = {"🏊": "#0984E3", "🚴": "#00B894", "🏃": "#E17055"}.get(seg["icon"], "#636E72")
        st.markdown(f"""
        <div style="background:var(--bg-card); border-left:4px solid {border_color}; border-radius:0 12px 12px 0; padding:16px; margin-bottom:12px;">
            <div style="display:flex; justify-content:space-between; align-items:center;">
                <div>
                    <span style="font-size:1.3rem;">{seg['icon']}</span>
                    <strong style="font-size:1.1rem; margin-left:8px;">{seg['name']}</strong>
                </div>
                <div style="text-align:right;">
                    <div style="font-weight:700; color:{border_color}; font-size:1.2rem;">{seg['target']}</div>
                    <div style="font-size:0.8rem; color:var(--text-muted);">{seg['pace']}</div>
                </div>
            </div>
            <div style="margin-top:8px; font-size:0.9rem; color:var(--text-secondary);">{seg['strategy']}</div>
        </div>
        """, unsafe_allow_html=True)

with tab2:
    st.markdown("### ⏱️ Detailed Pacing Strategy")
...
with tab4:
    st.markdown("### ⚠️ Contingency Plans")
    ...
    contingencies = [
        ...
        {"problem": "Extreme heat (>35°C)", "severity": "🟡 Medium",
    ...
    for c in contingencies:
        with st.expander(f"{c['severity']} {c['problem']}"):
